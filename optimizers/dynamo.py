...
#             if len(grads) == 0:
#                 continue
#             energy = torch.mean(p.grad.data**2 for p in group['params'] if p.grad is not None)
#             coherence = torch.mean(torch.abs(torch.tanh(p.grad.data for p in group['params'] if p.grad is not None)))

#             # homeostatic modulation
#             coherence_error = coherence - coherence_target
#             homeo_mod = 1.0 - homeo_rate * torch.tanh(coherence_error)
...
class BiostatisV6(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            ).item()  # Note: 0.5× weaker than before
